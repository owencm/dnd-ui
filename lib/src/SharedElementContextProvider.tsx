...
import {
  ...
  GHOST_LAYER_CLEAR_TIMEOUT,
  DEBOUNCE_TIMEOUT,
} from "./constants";
import {
  getKeyframes as getDefaultKeyframes,
  getNode as getDefaultNode,
} from "./utils";
...
export default function ShareElementContextProvider({
  ...
  const [sharedElements, setSharedElements, sharedElementsRef] = useStateRef<
      ...
      setSharedElements((prevSharedElements) => {
        ...
        return {
          ...
          [id]: {
            lastSeenAtPathname,
            // @ts-expect-error
            firstBoundingClientRect: ref.firstChild.getBoundingClientRect(),
            node: ref,
            id,
          },
        };
  ...
  const maybeTransition = useCallback(async () => {
    ...
    if (sharedElementsToTransition.length) {
      return Promise.all(
        sharedElementsToTransition.map(({ animation }) => animation.finished)
  ...
  const mountSharedElement = useCallback(
    (
      ...
      if (!sharedElementsRef.current[sharedElement.id]) {
        addSharedElement(sharedElement, pathnameOfSharedElement);
      } else if (
        sharedElementsRef.current[sharedElement.id]?.lastSeenAtPathname !==
        pathnameOfSharedElement
      ) {
        updateSharedElement(sharedElement, pathnameOfSharedElement);
        setIsTransitioning(true);
      }
    },
    [addSharedElement, updateSharedElement]
  );
